...
class Block(nn.Module):
    ...
    """
    def __init__(self, dim, P, drop_path=0., layer_scale_init_value=1e-6, dcls_kernel_size=17,
...
class ConvNeXt_dcls(nn.Module):
    ...
    def __init__(self, in_chans=3, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
                 drop_path_rate=0., layer_scale_init_value=1e-6, out_indices=[0, 1, 2, 3], dcls_kernel_size=17,
                 dcls_kernel_count=34, dcls_sync=True
                 ):
        super().__init__()
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=dims[i], P=self.P_stages[i], drop_path=dp_rates[cur + j],
                        layer_scale_init_value=layer_scale_init_value, dcls_kernel_size=dcls_kernel_size,
                        dcls_kernel_count=dcls_kernel_count) for j in range(depths[i])]
            )
